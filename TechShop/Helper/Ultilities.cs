    ...
    public static class Utilities
    {
        ...
        public static async Task<string?> UploadFile(Microsoft.AspNetCore.Http.IFormFile file, string sDirectory, string? newname = null)
        {
            try
            {
                ...
                string pathFile = Path.Combine(path, newname); // Simplified path construction
                var supportedTypes = new[] { "jpg", "jpeg", "png", "gif" };
                var fileExt = Path.GetExtension(file.FileName).Substring(1);
                if (!supportedTypes.Contains(fileExt.ToLower()))
                {
                    return null; // Consider throwing an exception or handling this case explicitly
                }
                else
                {
            ...
            {
                return null; // Consider logging the exception or handling it more explicitly
            }
        }
    }
}